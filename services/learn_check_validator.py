...
def learn_check_validator(user: User, serializer: CheckLearnAimSerializer, is_create=True) -> None:
    ...
    raises:
    - LearnAimAlreadyChecked: if the learn aim is already checked
    - LearnAimStageCantStartHigherThenOne: if the learn aim stage is higher than 1
    - LearnCheckNotApproved: if the previous learn check is not approved
    - SemesterCantBeLowerThenPrevious: if the semester is lower than the current semester on Learn Check request
    ...
    for check in previous_checks:
        if is_create:
            if check.close_stage < serializer.validated_data[('close_s'
                                                              'tage')] and not check.is_approved:
